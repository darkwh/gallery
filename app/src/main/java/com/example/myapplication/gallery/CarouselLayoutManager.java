...
@SuppressWarnings({"WeakerAccess", "unused", "AliControlFlowStatementWithoutBraces"})
public class CarouselLayoutManager extends ViewPagerLayoutManager {
    ...
    @Override
    protected void setItemViewProperty(View itemView, float targetOffset, int position) {
        float scale = calculateScale(targetOffset + mSpaceMain);
        int width = itemView.getWidth();
        int changeWidth = (int) (itemView.getWidth() * scale);
        int distance = (width - changeWidth) / 2;
        itemView.setScaleX(scale);
        itemView.setScaleY(scale);
        int centerPosition = getCurrentPosition();
        if (centerPosition - 2 == position) {
            itemView.setTranslationX(distance);
        } else if (centerPosition + 2 == position) {
            itemView.setTranslationX(-distance);
        }
    }
