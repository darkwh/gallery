...
@SuppressWarnings({"WeakerAccess", "unused", "SameParameterValue", "AliControlFlowStatementWithoutBraces"})
public abstract class ViewPagerLayoutManager extends LinearLayoutManager {
    ...
    protected abstract void setItemViewProperty(View itemView, float targetOffset, int postion);

    /**
     * cause elevation is not support below api 21,
     * so you can set your elevation here for supporting it below api 21
     * or you can just setElevation in {@link #setItemViewProperty(View, float, int)}
     */
    ...
    private void layoutItems(RecyclerView.Recycler recycler) {
        ...
        for (int i = start; i < end; i++) {
            if (useMaxVisibleCount() || !removeCondition(getProperty(i) - mOffset)) {
                ...
                layoutScrap(scrap, targetOffset, i);
    ...
    /* package */ float getMaxOffset() {
        return !mShouldReverseLayout ? (getItemCount() - 3) * mInterval : 0;
    }

    /* package */ float getMinOffset() {
        return !mShouldReverseLayout ? 2 * mInterval : -(getItemCount() - 3) * mInterval;
    }

    private void layoutScrap(View scrap, float targetOffset, int position) {
        ...
        setItemViewProperty(scrap, targetOffset, position);
    }
     ...
     */
    public float getOffsetOfRightAdapterPosition() {
